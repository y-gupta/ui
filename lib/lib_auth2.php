...
function can_access($perms,$block=true,$login2_url='login')//returns true if given permissions are with the current user. if block=true, terminates the execution.
{
	if((user('level')&$perms)!==$perms)
	{
		...
		if($block)
		{
			...
			}else{?><div class="alert alert-danger">Did you forget to do something?<br/><a href='<?php echo $login2_url ?>?redir=<?php echo urlencode($_SERVER['REQUEST_URI']) ?>'>Please Login</a> to Continue</div><?php
			}
			exit();
		}
		return false;
...
function logged_in()
{
	if(!session_id())
		session_start();
	if(isset($_SESSION[IID.'_login2_key'])&&isset($_SESSION[IID.'_login2_time'])&&isset($_SESSION[IID.'_login2_user']))
	{
		$timestamp=$_SESSION[IID.'_login2_time'];
		$user=$_SESSION[IID.'_login2_user'];
		$users=\ui\config('auth2_users');
		if(isset($users[$user]))
		{
			$key=make_key($user,$users[$user]['password'],$timestamp);
			if($key===$_SESSION[IID.'_login2_key'])
			{
				return $user;
			}
		}
	}elseif(isset($_COOKIE[IID.'_login2_key'])&&isset($_COOKIE[IID.'_login2_time'])&&isset($_COOKIE[IID.'_login2_user']))
	{
		$timestamp=$_COOKIE[IID.'_login2_time'];
		$user=$_COOKIE[IID.'_login2_user'];
 	$users=\ui\config('auth2_users');
		if(isset($users[$user]))
		{
			$key=make_key($user,$users[$user]['password'],$timestamp);
			if($key===$_COOKIE[IID.'_login2_key'])
			{
				$_SESSION[IID.'_login2_time']=$timestamp;
				$_SESSION[IID.'_login2_key']=$key;
				$_SESSION[IID.'_login2_user']=$user;
				return $user;
			}
		}
	}
